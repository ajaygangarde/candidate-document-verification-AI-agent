...
def _compute_verdict(extracted: float, expected: float) -> tuple[VerificationVerdict, float]:
    discrepancy = round(extracted - expected, 1)
    if extracted == 0:
        return VerificationVerdict.INSUFFICIENT_DATA, 0.0
    if abs(discrepancy) <= 0.5:
        return VerificationVerdict.MATCH, discrepancy
    return VerificationVerdict.DISCREPANCY, discrepancy
...
def verify(text: list[str], expected_years: float) -> ExperienceVerification:
    ...
    gap_years = expected_years - total_years
    gap_months = round(gap_years * 12, 1)
    ...
    return ExperienceVerification(
        ...
        verdict=verdict,
        discrepancy=discrepancy if verdict == VerificationVerdict.DISCREPANCY else None,
        confidence=0.9 if verdict != VerificationVerdict.INSUFFICIENT_DATA else 0.2,
        notes=f"Found {len(experience.roles)} role(s). Total: {total_years} yrs vs expected {expected_years} yrs.",
        gap_months=gap_months,
        gap_flag=gap_flag,
